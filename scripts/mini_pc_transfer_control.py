...
<div class='card'><h2>Transfer Status</h2><p>Shows the newest Rocky → Mini-PC transfer job. Use Refresh Status if the browser missed an update.</p><button onclick='refreshTransferStatus()'>Refresh Status</button><div id='transfer-status'><small>Loading…</small></div></div>
...
let lastJobId=null;
async function startTransfer(){{try{{const paths=[...document.querySelectorAll('input[type=checkbox]:checked')].map(x=>x.value); if(!paths.length) throw new Error('Select at least one file/folder'); const data=await j('/api/transfer',{{method:'POST',headers:{{'Content-Type':'application/json'}},body:JSON.stringify({{paths}})}}); lastJobId=data.job&&data.job.id; log(data); renderTransferStatus(data.job); if(data.job) poll(data.job.id)}}catch(e){{log(e.message)}}}}
async function poll(id){{const data=await j('/api/jobs/'+id); lastJobId=id; log(data); renderTransferStatus(data); if(data.status==='running') setTimeout(()=>poll(id),2000); else if(data.status==='completed' && data.verified && !data.deleted) showDeleteButton(id)}}
function renderTransferStatus(job){{const box=document.getElementById('transfer-status'); if(!job){{box.innerHTML='<small>No transfer jobs yet.</small>'; return}}; const cls=job.status==='completed'?'ok':(job.status==='failed'||job.status==='verify_failed')?'bad':''; const logs=(job.logs||[]).slice(-8).map(esc).join('\n'); box.innerHTML=`<p class="${{cls}}"><strong>${{esc(job.status)}}</strong> · ${{job.percent||0}}% · ${{esc(job.transferred_human||'—')}}/${{esc(job.total_human||'—')}} · skipped ${{esc(job.skipped_human||'0 B')}}</p><p><small>Current: ${{esc(job.current||'—')}}</small></p><pre>${{logs}}</pre>`}}
async function refreshTransferStatus(){{try{{const data=await j('/api/jobs'); const job=(data.jobs&&data.jobs[0])||null; if(job) lastJobId=job.id; renderTransferStatus(job); if(job) log(job)}}catch(e){{document.getElementById('transfer-status').innerHTML='<p class="bad">'+esc(e.message)+'</p>'}}}}
...
loadRemoteStatus();
setInterval(refreshTransferStatus, 10000);
</script></body></html>"""
